...
function saveDataAntaresByDeviceId1() {
    global $conn;
    ...
    foreach ($serialNumbers as $serialNumber) {
        $deviceUrl = "https://platform.antares.id:8443/~/antares-cse/antares-id/SmartWaterMeter_Depok/$serialNumber?ty=4&fu=1&drt=2&lim=5";
        ...
        if (isset($deviceDataParsed['m2m:cin']) && is_array($deviceDataParsed['m2m:cin'])) {
            foreach ($deviceDataParsed['m2m:cin'] as $dataEntry) {
                $conParsed = json_decode($dataEntry['con'], true);
                $payloadValue = $conParsed['data'];
                $devEuiValue = $conParsed['devEui'];
                $radio = $conParsed['radio']['hardware'];
                $RSSI = $radio['rssi'];
                $SNR = $radio['snr'];
                $timestamp = convertAntaresTimeToTimestamp($dataEntry['ct']);
    
                $insertQuery = "INSERT INTO payload_device_depok (serial_number, payload, devEUI, rssi, snr, timestamp) 
                                VALUES ('$serialNumber', '$payloadValue', '$devEuiValue', '$RSSI', '$SNR', '$timestamp')";
